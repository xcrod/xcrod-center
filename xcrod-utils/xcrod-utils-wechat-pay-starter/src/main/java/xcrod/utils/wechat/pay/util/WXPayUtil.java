...
public class WXPayUtil {
    ...
    /**
     * 判断签名是否正确，必须不包含sign字段，否则返回false。使用MD5签名。
     *
     * @param data Map类型数据
     * @param key  API密钥
     * @return 签名是否正确
    ...
    /**
     * 判断签名是否正确，必须不包含sign字段，否则返回false。
     *
     * @param data     Map类型数据
     * @param key      API密钥
     * @param signType 签名方式
     * @return 签名是否正确
     */
    public static boolean isSignatureValid(Map<String, String> data, String key, WXPayConstants.SignType signType) throws NoSuchAlgorithmException, InvalidKeyException, WXPayException {
        if (!data.containsKey(WXPayConstants.FIELD_SIGN)) {
            return false;
        }
        String sign = data.get(WXPayConstants.FIELD_SIGN);
     ...
     * @return 签名
     */
    public static String generateSignature(final Map<String, String> data, String key, WXPayConstants.SignType signType) throws InvalidKeyException, NoSuchAlgorithmException, WXPayException {
...
//        Arrays.sort(keyArray);
        ...
        data.forEach((k, v) -> {
            if (!k.equals(WXPayConstants.FIELD_SIGN) && v.trim().length() > 0)
...
//        }
        ...
        if (WXPayConstants.SignType.MD5.equals(signType)) {
            ...
            throw new WXPayException("签名类型错误");
        }
    }
